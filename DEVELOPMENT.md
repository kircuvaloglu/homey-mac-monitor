...
Points from the first review, worth keeping in mind for every release:

- `README.txt` is one or two plain paragraphs about why the app is useful. No
  feature or Flow card lists, no setup commands, no URLs (use `homepage` and
  `source` in the manifest instead).
- `description` is a short tagline. It must not repeat the app name or list
  features.
- The app image is a real lifestyle photo, the driver image a real photo of the
  device on white. Illustrations are rejected. Images must not show Homey
  devices or logos.
- Widget previews are simple shapes on a transparent background, without text.
- Icons are line drawings without gradients.

Photos are turned into store images with:

```bash
swift tools/cutout.swift mac-photo.jpg driver.png
python3 tools/make-images.py app driver-photo driver.png
python3 tools/make-images.py app app-photo desk-photo.jpg
```
